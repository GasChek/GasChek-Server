...
class PaymentAPI(APIView):
    def post(self, request):
        ...
        if (
            not user.email
            or len(user.email) == 0
            or len(user.first_name) == 0
            or len(user.last_name) == 0
            or len(user.address) == 0
            or len(user.phonenumber) == 0
            or len(user.state) == 0
        ):
        ...
        if not cylinder_price:
            return Response({"status": 400, "message": "Gas Dealer is inactive."})

        gas_dealer = Gas_Dealer.objects.get(id=dealer)

        if gas_dealer.open is False or gas_dealer.selling is False:
            return Response({"status": 400, "message": "Gas Dealer is inactive."})
        ...
        try:
            ...
            if payment["status"] is True:
                ...
                return Response(
                    {"status": 200, "url": payment["data"]["authorization_url"]}
                )

            else:
                return Response({"status": 400, "message": "Invalid payment"})
        except Exception:
            return Response(
                {"status": 400, "message": "Error processing payment, try again."}
            )
        ...
        try:
            ...
            if ip in settings.PAYSTACK_SAFE_IPS:
                if request.data["event"] == "charge.success":
                    ...
                    gas_dealer = Gas_Dealer.objects.get(id=payment.gas_dealer.id)
                    gas_dealer.sold += 1
                    gas_dealer.save()
